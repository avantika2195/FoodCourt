  ...
  return (
    <>
      ...
      <div className='hidden lg:flex max-w[1520px] m-auto py-2 px-2'>
        <Splide options={
          {perPage:4,gap:'0.5rem',drag:'free',arrows:'false'}
        }
        > 
        {
          topPicks.map((item)=>{
            return(
              ...
              <div className='rounded-3xl relative'>
                <div className='absolute w-full h-full bg-black/50 rounded-3xl text-white'>
                  ...
                  <button className='border-white text-white mx-2 absolute bottom-4'>Add To Cart</button>
                </div>
                <img className='h-[200px] w-full object-cover rounded-3xl cursor-pointer hover:scale105 ease-out duration-300'
                src={item.img}
                alt={item.title}
                />
